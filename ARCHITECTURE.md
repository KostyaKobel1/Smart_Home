...
- **Reset Data модальне вікно:**
  - Select з двома режимами: event-log, factory
  - Підказки для кожного режиму
  - Окремі підтвердження для кожного режиму
...
- **component-action.css** - форми, кнопки, стилі кнопок за типами, фільтр кімнат з відступом та border-top
...
2. Ввести назву компонента (наприклад, "Ceiling Light", "Front Door Lock")
3. Вибрати тип компонента зі списку
4. (Опціонально) Вказати кімнату або вибрати з підказок (Living Room, Kitchen, Bedroom...)
5. Натиснути "Create Component"
6. Натиснути "Get List Of Components" для перегляду
7. Використати фільтр "Filter by Room" для фільтрації компонентів по кімнатах
8. Розгорнути панель компонента для керування діями
9. У статистиці використати кнопку "Show Room Only" / "Show Global" для перемикання режимів відображення
