...
- **Автоматичне оновлення списку компонентів** при створенні/видаленні (якщо список відкритий)
...
5. **Додатковий функціонал:**
   ...
   - Event Log (журнал останніх 50 подій)
   - **Smart Reset:** перевірка наявності даних перед видаленням
     - Якщо даних немає → показує toast, не дозволяє очищати
     - Якщо дані є → показує confirm діалог перед видаленням
...
- **Auto-refresh статистики** - статистика автоматично оновлюється при відкритій панелі
- **Auto-refresh списку компонентів** - список автоматично оновлюється при створенні/видаленні, якщо він відкритий
- **Smart Reset** - перевірка наявності даних перед видаленням (попередження при порожній системі)
